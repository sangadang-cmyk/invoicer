...
public interface InvoiceUserController {
    ...
    @GetMapping("/batch")
    List<InvoiceResponseDto> getAllInvoices(@AuthenticationPrincipal UserDetails principal);

    @Operation(summary = "Get my invoice by ID")
    @GetMapping("/{invoiceId}")
    InvoiceResponseDto getInvoiceById(@PathVariable String invoiceId, @AuthenticationPrincipal UserDetails principal);

    @Operation(summary = "Start processing an invoice")
    @PutMapping("/{invoiceId}/start")
    ImageUploadAttemptDto startProcessingInvoice(@PathVariable String invoiceId, @RequestBody StartImageUploadInvoiceCommand command, @AuthenticationPrincipal UserDetails principal);
}
